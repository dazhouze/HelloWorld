...
if __name__ == '__main__':
	...
	y = np.array([236.4, 234.4, 252.8, 298.6, 314.2, 342.2, 360.8, 378.0, 391.2, 390.8])[:, np.newaxis]
	lr = LinearRegression()
	pr = LinearRegression()
	...
	# plot
	fig = plt.figure()
	plt.scatter(X, y, label='training potints', color='lightgray')
	plt.plot(X_fit, y_lin_fit, label='linear (d=1), $R^2=%.2f$' % linear_r2, color='blue', lw=2, linestyle=':')
	plt.plot(X_fit, y_quad_fit, label='quadratic (d=2), $R^2=%.2f$' % linear_r2, color='red', lw=2, linestyle='-')
	plt.plot(X_fit, y_cubic_fit, label='cubic (d=3), $R^2=%.2f$' % linear_r2, color='green', lw=2, linestyle='--')
	plt.xlabel('% lower states of the popultion [LSTAT]')
	plt.ylabel('Price in $1000s [MEDV]')
	plt.legend(loc='upper right')
	fig.savefig('polynomial.pdf')
