...
def BinaryEulerTour(self):
	...
	def excute(self):
		...
		if len(self.__tree) > 0:
			return self.__tour(self.__tree.root(), 0, [])

	def __tour(self, p, d, path):
		'''
		...
		if self.__tree.left(p) is not None:
			path.append(0)
			results[0] = self.__tour(self.__tree.left(p), d+1, path)
			path.pop()
		self.__hook_invisit(p, d, path)
		if self.__tree.right(p) is not None:
			path.append(1)
			result[1] = self.__tour(self.__tree.right(p), d+1, path)
			path.pop()
